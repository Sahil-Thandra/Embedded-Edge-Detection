     ...
     /* Input: image1[y][x] ---- Outout: image2[y][x] */
{
  ...
  for (y = 0; y < y_size2; y++) {
    for (x = 0; x < x_size2; x++) {
	//printf("%d ",image2[y][x]);
	if(image2[y][x]>=200||image2[y][x]<=50)
      		image2[y][x] = 0;
	else
		image2[y][x]=255;
    }
...
///////////////////////////////////////////////////////
  for (y = 1; y < y_size1 - 1; y++) {
    for (x = 1; x < x_size1 - 1; x++) {
      ...
      for (j = -1; j <= 1; j++) {
	    for (i = -1; i <= 1; i++) {
	      pixel_value += weight1[j + 1][i + 1] * image1[y + j][x + i];
	    }
      }
      if (pixel_value < min) min = pixel_value;
      if (pixel_value > max) max = pixel_value;
  ...
  for (y = 1; y < y_size1 - 1; y++) {
    for (x = 1; x < x_size1 - 1; x++) {
      ...
      pixel_value = MAX_BRIGHTNESS * (pixel_value - min) / (max - min);
      image3[y][x] = (unsigned char)pixel_value;
    }
  }
  for (y = 0; y < y_size2; y++) {
    for (x = 0; x < x_size2; x++) {
	//printf("%d ",image2[y][x]);
	if(image3[y][x]>=200||image3[y][x]<=50)
      		image3[y][x] = 0;
	else
		image3[y][x]=255;
    }
